...
@section('content')
<h2 style="color: #1e40af; margin-bottom: 1.5rem; font-weight: 700;">Editar Proyecto</h2>
...
<form action="{{ route('proyectos.update', $proyecto->id) }}" method="POST" style="background-color: #f9fafb; padding: 2rem; border-radius: 10px; max-width: 600px; box-shadow: 0 4px 8px rgba(0,0,0,0.05);">
    @csrf
    @method('PUT')
    ...
    <div style="margin-bottom: 1.5rem;">
        ...
        >
            ...
            @foreach ($planes as $plan)
                <option value="{{ $plan->id }}" {{ (old('plan_id', $proyecto->plan_id) == $plan->id) ? 'selected' : '' }}>
                    {{ $plan->nombre }}
                </option>
            @endforeach
        </select>
