...
<div style="max-width: 1100px; margin: 30px 0 30px 10px; padding: 0 10px; font-family: Arial, sans-serif;">
  <h1 style="margin-bottom: 20px; color: #1e40af;">Listado de {{ $nombreTipo }}</h1>

  <a href="{{ route('objetivos.create', ['tipo' => $tipo]) }}" 
    ...
    <i class="fas fa-plus" aria-hidden="true"></i> Crear Nuevo Objetivo
  </a>
  ...
  <table style="width: 100%; border-collapse: collapse; margin-bottom: 30px;">
    <thead>
      <tr style="background-color: #2563eb; color: white;">
        <th class="oculto" style="padding: 12px 15px; text-align: left; min-width: 50px;">ID</th>
        ...
        <th style="padding: 12px 15px; text-align: left; min-width: 90px;">Tipo</th>
        <th style="padding: 12px 15px; text-align: left; min-width: 90px;">Estado</th>
        <th style="padding: 12px 15px; min-width: 130px;">Fecha Registro</th>
    ...
    <tbody>
      @forelse($objetivos as $objetivo)
        <tr style="border-bottom: 1px solid #ddd;">
          <td class="oculto" style="padding: 10px 15px;">{{ $objetivo->id }}</td>
          <td style="padding: 10px 15px;">{{ $objetivo->codigo }}</td>
          <td style="padding: 10px 15px;" title="{{ $objetivo->nombre }}">{{ $objetivo->nombre }}</td>
          <td style="padding: 10px 15px;" title="{{ $objetivo->descripcion }}">{{ $objetivo->descripcion }}</td>
          <td style="padding: 10px 15px;">{{ ucfirst($objetivo->tipo) }}</td>
          <td style="padding: 10px 15px;">{{ ucfirst($objetivo->estado) }}</td>
      ...
      @empty
        <tr>
          <td colspan="8" style="text-align: center; padding: 1rem;">No hay objetivos registrados.</td>
        </tr>
      @endforelse
    </tbody>
  </table>
  ...
  @if ($objetivos->hasPages())
    <nav aria-label="Paginación">
      <ul style="list-style: none; display: flex; justify-content: center; gap: 12px; padding: 0;">
        @if ($objetivos->onFirstPage())
          <li style="color: #aaa; padding: 8px 16px; border: 1px solid #ccc; border-radius: 4px;">Anterior</li>
        @else
          <li>
            <a href="{{ $objetivos->previousPageUrl() }}" style="text-decoration:none; color:#2563eb; padding: 8px 16px; border: 1px solid #2563eb; border-radius: 4px;">
              ← Anterior
            </a>
          </li>
        @endif

        @if ($objetivos->hasMorePages())
          <li>
            <a href="{{ $objetivos->nextPageUrl() }}" style="text-decoration:none; color:#2563eb; padding: 8px 16px; border: 1px solid #2563eb; border-radius: 4px;">
              Siguiente →
            </a>
          </li>
        @else
  ...
  <script>
    ...
    const modalObjetivoNombre = document.getElementById('modal-objetivo-nombre');
