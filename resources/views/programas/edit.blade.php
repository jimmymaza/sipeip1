...
@section('content')
<h2 style="color: #1e40af; margin-bottom: 1.5rem; font-weight: 700;">Editar Programa</h2>
...
<form action="{{ route('programas.update', $programa->id) }}" method="POST" style="background-color: #f9fafb; padding: 2rem; border-radius: 10px; max-width: 600px; box-shadow: 0 4px 8px rgba(0,0,0,0.05);">
    @csrf
    @method('PUT')
    ...
    <div style="margin-bottom: 1.5rem;">
        ...
        >
            ...
            @foreach ($planes as $plan)
                <option value="{{ $plan->id }}" {{ (old('plan_id', $programa->plan_id) == $plan->id) ? 'selected' : '' }}>
                    {{ $plan->nombre }}
                </option>
            @endforeach
        </select>
    ...
    <div style="margin-bottom: 1.5rem;">
        ...
        <input 
            ...
            name="fecha_inicio" 
            id="fecha_inicio" 
            value="{{ old('fecha_inicio', $programa->fecha_inicio) }}" 
            required 
            aria-required="true"
    ...
    <div style="margin-bottom: 1.5rem;">
        ...
        <input 
            ...
            name="fecha_fin" 
            id="fecha_fin" 
            value="{{ old('fecha_fin', $programa->fecha_fin) }}" 
            required 
            aria-required="true"
            style="width: 100%; padding: 0.6rem 0.8rem; border: 1.8px solid #d1d5db; border-radius: 8px;"
        >
